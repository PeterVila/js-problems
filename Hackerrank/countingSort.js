function countingSort(arr) {
    ...
    console.log('newArr after', newArr) //Test case does .join(' ') for us.
    output = newArr
    return output;

    /* Create an integer array with a index range that covers the range of values in your array to sort, everytime a value occurs, increment that counter at the index.

    Create a new list that always returns 100 elements (ALWAYS READ NOTE)
    -Fill in the elements with the value 0
    -Go through every element in the original array
        -At every value in our original array, increment that index in our new array
    -Return the output;
